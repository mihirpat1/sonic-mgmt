...
| voltage_range | dict | (format) {"upper": <float>, "lower": <float>} | O | transceivers | Allowed deviation window in volts between baseline and post-test `voltage` reading. Format: `{"upper": <float>, "lower": <float>}` where `upper` is the max allowed increase and `lower` is the max allowed decrease from baseline. Omit to disable this post-test deviation check. |
| laser_temperature_range | dict | (format) {"upper": <float>, "lower": <float>} | O | transceivers | Allowed deviation window in Celsius between baseline and post-test `laser_temperature` reading. Format: `{"upper": <float>, "lower": <float>}`. Omit to disable this post-test deviation check. |
| txLANE_NUMbias_range | dict | (format) {"upper": <float>, "lower": <float>} | O | transceivers | Allowed deviation window in mA between baseline and post-test `tx{lane}bias` reading for each lane. Format: `{"upper": <float>, "lower": <float>}`. Omit to disable this per-lane post-test deviation check. |
| txLANE_NUMpower_range | dict | (format) {"upper": <float>, "lower": <float>} | O | transceivers | Allowed deviation window in dBm between baseline and post-test `tx{lane}power` reading for each lane. Format: `{"upper": <float>, "lower": <float>}`. Omit to disable this per-lane post-test deviation check. |
| rxLANE_NUMpower_range | dict | (format) {"upper": <float>, "lower": <float>} | O | transceivers | Allowed deviation window in dBm between baseline and post-test `rx{lane}power` reading for each lane. Format: `{"upper": <float>, "lower": <float>}`. Omit to disable this per-lane post-test deviation check. |
...
**Post-test deviation rule:** For tests that capture baseline DOM values and then restore the port to steady-state operation, compare the post-test DOM values against the baseline for each configured deviation attribute above. Each attribute is a dict with `"upper"` (max allowed increase) and `"lower"` (max allowed decrease) keys. The test fails if the post-test value exceeds `baseline + upper` or falls below `baseline - lower` for any enabled field. Lane-based entries such as TX bias and TX/RX power use the `LANE_NUM` expansion and are validated per lane.
      ...
      "mellanox": {
        "part_numbers": {
          ...
          "MMA1T00-VS-400G": {
            ...
            "voltage_range": {"upper": 0.05, "lower": 0.05},
            "laser_temperature_range": {"upper": 3.0, "lower": 3.0},
            "txLANE_NUMbias_range": {"upper": 2.0, "lower": 2.0},
            "txLANE_NUMpower_range": {"upper": 1.5, "lower": 2.0},
            "rxLANE_NUMpower_range": {"upper": 2.0, "lower": 2.0}
          }
        }
      },
      "marvell": {
