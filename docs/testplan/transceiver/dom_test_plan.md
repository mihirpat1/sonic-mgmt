...
| voltage_range | dict | - | O | transceivers | Allowed deviation window in volts between baseline and post-test `voltage` reading. Format: `{"upper": <float>, "lower": <float>}` where `upper` is the max allowed increase and `lower` is the max allowed decrease from baseline. Omit to disable this post-test deviation check. |
| laser_temperature_range | dict | - | O | transceivers | Allowed deviation window in Celsius between baseline and post-test `laser_temperature` reading. Format: `{"upper": <float>, "lower": <float>}`. Omit to disable this post-test deviation check. |
| txLANE_NUMbias_range | dict | - | O | transceivers | Allowed deviation window in mA between baseline and post-test `tx{lane}bias` reading for each lane. Format: `{"upper": <float>, "lower": <float>}`. Omit to disable this per-lane post-test deviation check. |
| txLANE_NUMpower_range | dict | - | O | transceivers | Allowed deviation window in dBm between baseline and post-test `tx{lane}power` reading for each lane. Format: `{"upper": <float>, "lower": <float>}`. Omit to disable this per-lane post-test deviation check. |
| rxLANE_NUMpower_range | dict | - | O | transceivers | Allowed deviation window in dBm between baseline and post-test `rx{lane}power` reading for each lane. Format: `{"upper": <float>, "lower": <float>}`. Omit to disable this per-lane post-test deviation check. |
